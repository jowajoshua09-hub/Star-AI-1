import os, re, json, threading, aiohttp, urllib.parse
...
def clean_prompt(t):
    ...
    p = re.sub(r'^(image|video) of\s+', '', p, flags=re.I).strip()
    return p or t
...
async def brain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_video_request(text):
        await update.message.reply_text(
            f"Hey {user_name}, I don't do videos for now to stay fast ⚡\nBut I can make a fire image! Try: generate image of {clean_prompt(text)}"
        )
        return
    ...
    try:
        if is_image(text):
            ...
            try:
                ...
                await update.message.reply_text(f"Oops {user_name}, can't get that image right now, try another prompt? 😅")
            return

        try:
            reply = await get_ai_reply(user_name, text)
            markup = get_owner_buttons() if is_owner_q(text) else None
            await placeholder.edit_text(reply[:4000], reply_markup=markup)
        except:
            await placeholder.edit_text(f"Oops {user_name}, my brain lagged a bit, can you say that again? 😅")

    except:
        try:
            await placeholder.edit_text(f"Oops {user_name}, something tripped, try again my G?")
        except:
            try:
                await update.message.reply_text(f"Oops {user_name}, can't get that right now, let's try again?")
            except:
                pass
...
def main():
    ...
    print("Star AI Ready - No Video, No 404, Name Memory ON")
    app.run_polling()
