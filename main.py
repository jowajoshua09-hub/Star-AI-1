...
async def start_cmd(u,c):
    await u.message.reply_text(
        f"Hey {remember(u.effective_user)} 🔥\n"
...
async def brain(update,context):
    ...
    # Check if the text starts with common image generation phrases
...
@flask_app.route('/')
def home(): return f"✅ {OWNER_NAME} Live - YT Fixed + Image Gen"
@flask_app.route('/docs')
def docs(): return f"<h1>⭐ Star AI API by {OWNER_NAME}</h1><p>GET /api/ai?key={API_KEY}&message=hi</p><p>GET /api/image?key={API_KEY}&prompt=cat</p>"
...
async def main():
    ...
    app.add_handler(CommandHandler("imagine",imagine_cmd))  # keep for slash compatibility
